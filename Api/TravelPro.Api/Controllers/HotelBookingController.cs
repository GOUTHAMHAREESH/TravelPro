...
namespace TravelPro.Api.Controllers
{
    ...
    public class HotelBookingController : ApiController
    {
        ...
        public IHttpActionResult AddHotelBooking(HotelBookingDto dataDto)
        {
            if (dataDto != null)
            {
                var today = DateTime.Today;
                if (dataDto.Id <= 0 && (dataDto.FromDate.Date < today || dataDto.ToDate.Date < today || dataDto.ToDate.Date < dataDto.FromDate.Date))
                {
                    return BadRequest("Past dates are not allowed. Please select current or future dates.");
                }
                ...
                using (TravelProDB context = new TravelProDB())
                {
                    if (dataDto.Id <= 0)
                    {
                        var requestedFrom = dataDto.FromDate.Date;
                        var requestedTo = dataDto.ToDate.Date;

                        var hasConflict = context.HotelBookings.Any(x =>
                            x.IsActive &&
                            x.HotelRoomId == dataDto.HotelRoomId &&
                            x.Id != dataDto.Id &&
                            x.Status != "Rejected" &&
                            x.Status != "Cancelled" &&
                            DbFunctions.TruncateTime(x.FromDate) <= requestedTo &&
                            DbFunctions.TruncateTime(x.ToDate) >= requestedFrom);

                        if (hasConflict)
                        {
                            return BadRequest("This room is already booked for the selected dates. Please choose different dates or another room.");
                        }
                    }
                    ...
                    if (dataDto.Id > 0)
                    {
                        ...
                        if (data != null)
                        {
                            ...
                            data.Status = string.IsNullOrWhiteSpace(dataDto.Status) ? data.Status : dataDto.Status;
                    ...
                    else
                    {
                        ...
                        data.FromDate = dataDto.FromDate;
                        data.ToDate = dataDto.ToDate;
                        data.Status = string.IsNullOrWhiteSpace(dataDto.Status) ? "Pending" : dataDto.Status;
