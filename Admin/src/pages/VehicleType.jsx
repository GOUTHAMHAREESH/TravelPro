...
import { vehicleTypeList, addVehicleType, getVehicleTypeById, deleteVehicleType } from '../api';
...
export default function VehicleType() {
  ...
  const loadList = async () => {
    setLoading(true);
    try {
      const data = await vehicleTypeList();
      setList(data || []);
    } catch (e) {
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      await addVehicleType({ Id: editingId || 0, Name: name });
      setShowForm(false);
      loadList();
    } catch (e) {
    ...
    <div className="data-page">
      ...
      <div className="data-table-wrap">
        {loading ? (
          ...
          <p className="empty-msg">No vehicle types found. Add one to get started.</p>
        ) : filteredList.length === 0 ? (
          <p className="empty-msg">No vehicle types match the filter.</p>
        ) : (
      ...
      {showForm && (
        <div className="form-modal-overlay" onClick={() => setShowForm(false)}>
          <div className="form-modal" onClick={(e) => e.stopPropagation()}>
            <h3>{editingId ? 'Edit Vehicle Type' : 'Add Vehicle Type'}</h3>
            <form onSubmit={handleSubmit}>
              <div className="form-group">
                <label>Name</label>
                <input value={name} onChange={(e) => setName(e.target.value)} required placeholder="Vehicle type name" />
              </div>
